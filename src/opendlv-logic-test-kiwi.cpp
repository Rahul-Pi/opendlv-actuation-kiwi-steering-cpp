...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid") || 0 == commandlineArguments.count("freq")) {
    ...
    auto onAimAngleReading{[&behavior](cluon::data::Envelope &&envelope)
    {
        auto AimAngleReading = cluon::extractMessage<opendlv::proxy::AngleReading>(std::move(envelope));
        behavior.setAimAngle(AimAngleReading);
    }};
    auto onKiwiDetection{[&behavior](cluon::data::Envelope &&envelope)
    {
        auto kiwiDetection = cluon::extractMessage<opendlv::logic::perception::KiwiDetection>(std::move(envelope));
        behavior.setKiwiDetect(kiwiDetection);
    }};
    auto onIntersectionDetection{[&behavior](cluon::data::Envelope &&envelope)
    {
        auto intersectionDetection = cluon::extractMessage<opendlv::proxy::SwitchStateRequest>(std::move(envelope));
        behavior.setIntersectionDetect(intersectionDetection);
    }};

    cluon::OD4Session od4{CID};
    od4.dataTrigger(opendlv::proxy::DistanceReading::ID(), onDistanceReading);
    od4.dataTrigger(opendlv::proxy::VoltageReading::ID(), onVoltageReading);
    od4.dataTrigger(opendlv::proxy::AngleReading::ID(), onAimAngleReading);
    od4.dataTrigger(opendlv::logic::perception::KiwiDetection::ID(), onKiwiDetection);
    od4.dataTrigger(opendlv::proxy::SwitchStateRequest::ID(), onIntersectionDetection);
